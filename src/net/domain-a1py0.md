...
![image](assets/image-20241111092527-walcsoh.png)
...
### 在工作组中
 ...
 ![在这里插入图片描述](assets/network-asset-e2f2dac2974531184c7b3ae7676e430b-20250107094537-kbfx7ag.png)
...
> 在域环境中，由于域控制器的存在（所有域用户的哈希值都存储在域控上的`NTDS.dit`中），服务器没办法直接进行“工作组验证流程”中的步骤④，只能作为“中间安全人”连通客户端和域控，并将验证过程交给域控。
 ...
 ![在这里插入图片描述](assets/network-asset-35da2520c71862b33ac426ef13f40577-20250107094538-dbhuzdh.png)
...
![image](assets/image-20241112092551-3t61u6o.png)
...
![在这里插入图片描述](assets/network-asset-f7e6ad2eeaa55ef26136a3b83578fc6c-20250107094538-hcu09v6.png)
...
![image](assets/image-20241112092814-yhu42b1.png)

## windows10入域

参考文章：Windows域环境搭建

1. 在服务器端先配置静态ip，DNS服务器​![image](assets/image-20241111144153-gmzzzcr.png)
2. 修改计算机全名，方便好记。​

    ![image](assets/image-20241111144402-lhk9zkk.png)
3. 安装服务器的角色  AD​![image](assets/image-20241111142624-d6wd8ws.png)
4. 安装完成后将该服务器升级为域控制器​![image](assets/image-20241111142540-xlcvuj0.png)
5. 安装过程如果没有密码要设置强密码​

    ![image](assets/image-20241111143408-w9ii1pc.png)
6. 完了之后就显示在域了​![image](assets/image-20241111144121-tmu4qqe.png)
7. 转到用户计算机windows10用户加入域，加入域之前需要先配置用户的IP和DNS等![image](assets/image-20241111145003-gg13umb.png)
8. 输入域管理账密后就可加入![image](assets/image-20241111145027-dg1d33s.png)
9. 加入之后重启计算机。
10. 在域控制服务器端可查看到用户已经添加进来。​![image](assets/image-20241111150337-h8rhhka.png)
11. 我们将该域用户加入到某部门​![image](assets/image-20241111150522-ww1ftlh.png)
12. 设置域用户的密码及相关策略。​![image](assets/image-20241111151204-sr2vag2.png)
13. 查看域用户的登录方式​

     ![image](assets/image-20241111151814-t08hsy7.png)
14. 输入域用户名登录​

     ![image](assets/image-20241111151620-zlj86d9.png)

## windows7入域

1. Windows7使用相同的方法创建和加入域​![image](assets/image-20241111160435-dberip3.png)​​![image](assets/image-20241111160613-j6m5xm1.png)​​![image](assets/image-20241111160927-3de4lza.png)​​![image](assets/image-20241111161924-akw6l9x.png)
2. 用户也可以在域控制器添加​

    ![image](assets/image-20241111162927-6kxprhl.png)

## 域相关命令

![image](assets/image-20241111165218-p78wz15.png)

![image](assets/image-20241111165155-p6nmq0h.png)

![image](assets/image-20241111185659-qzrvfhw.png)

net view /domain  #查看所有域

![image](assets/image-20241111184658-ed7oc4r.png)

net user /domain      #查看域用户

![image](assets/image-20241111184921-22bfn54.png)

net group /domain      #查看域里面有哪些组

![image](assets/image-20241111185005-fzwfy4e.png)

net group "domain admins" /domain #查看域管理员账户

![image](assets/image-20241111185114-1rpbkyr.png)

net group "domain controllers" /domain    #查看域控制器

![image](assets/image-20241111185158-wr9eiv1.png)

net group "domain computers" /domain     #查看加入域中的所有普通计算机名称

![image](assets/image-20241111185229-upusrh1.png)

![image](assets/image-20241111185628-vms3cis.png)

![image](assets/image-20241111185551-tq2kmi6.png)

dsquery server    #查询活动目录中的域控制器

dsquery use   r #查询活动目录所有用户，也就是域用户

![image](assets/image-20241111185727-xxaelse.png)

net config workstation    #查询当前计算机属于那个域

![image](assets/image-20241111190154-p516xb5.png)

net time /domain #查看主域服务器的时间

![image](assets/image-20241111190443-pmvwcvy.png)

Ipconfig /all

![image](assets/image-20241111190537-c8htfud.png)

net user xiaomi passwd /add /domain    #添加普通域用户xiaomi

net group "Domain Admins" xiaomi /add /domain    #将普通域用户提升为域管理员

![image](assets/image-20241111190938-6ocvh8w.png)
